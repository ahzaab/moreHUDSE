...
Function Maintenance()
    ...
    if SKSE.GetPluginVersion("Ahzaab's moreHUD Plugin") < 0
        Debug.MessageBox("moreHUD: The native SKSE plugin is not detected!")
        UnregisterForAllKeys()
        return
    endif
    ...
    UnregisterForModEvent("AHZmoreHUD_MovieLoaded")
    RegisterForModEvent("AHZmoreHUD_MovieLoaded", "OnMovieLoaded")

    if AhzMoreHud.IsMovieLoaded()
        UpdateSettings(false)
    endif

    Utility.Wait(1)

    ; Compatibility fallback for replacement SWFs that predate MovieLoaded().
    if !AhzMoreHud.IsMovieLoaded()
        UpdateSettings(false)
    endif
...
function UpdateSettings(bool disable)
    ...
    ; Keep key registration synchronized with the current setting.
    if disable
        UnregisterForAllKeys()
    else
...
event OnMovieLoaded(string a_eventName, string a_strArg, float a_numArg, Form a_sender)
    ; Confirm the retained state in case a queued event belongs to a HUD that closed.
    if AhzMoreHud.IsMovieLoaded()
        UpdateSettings(false)
    endif
EndEvent
...
Function RefreshHotkeyRegistration()
    UnregisterForAllKeys()
    if AHZHotKey && AHZHotKey.GetValueInt() >= 0
