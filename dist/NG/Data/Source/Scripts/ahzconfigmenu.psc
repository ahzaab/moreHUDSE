...
float Property DefaultIconSize = 20.0 autoReadOnly
...
event OnConfigOpen()
    ...
    ValidatePapyrusUtil = false
    int papyrusUtilPluginVersion = SKSE.GetPluginVersion("PapyrusUtil")
    if papyrusUtilPluginVersion < 0
        papyrusUtilPluginVersion = SKSE.GetPluginVersion("PapyrusUtil Plugin")
    endif
    if papyrusUtilPluginVersion >= 0
        int papyrusUtilDllVersion = PapyrusUtil.GetVersion()
        int papyrusUtilScriptVersion = PapyrusUtil.GetScriptVersion()
        ValidatePapyrusUtil = papyrusUtilDllVersion > 31 && papyrusUtilScriptVersion > 31
    endif
    ...
    selectedConfig = GetValidArrayIndex(selectedConfig, saConfigs, 0)
...
event OnConfigClose()
    ...
    if (_uninstallMod == true)
        if mainQuest
            mainQuest.Unregister()
        endif
        Stop()
    elseif mainQuest
        mainQuest.UpdateSettings(false)
    endIf
endEvent
...
event OnPageReset(string a_page)
    ...
    if !WidgetStyles2 || !AlignmentStyles || !ActivationModes
        OnConfigInit()
    endif

    if (a_page == "$mHUD_GeneralPage")
        AddHeaderOption("$mHUD_ActivationOptions")
        _activationModeOID_M    = AddMenuOption("$mHUD_ActivationMode", ActivationModes[GetValidActivationModeIndex(AHZActivationMode.GetValueInt())])
    ...
    elseif (a_page == "$mHUD_PlayersDataPage")
        ...
        _bottomAlignmentOID_M   = AddMenuOption("$mHUD_Alignment", AlignmentStyles[GetValidArrayIndex(AHZBottomWidgetRightAligned.GetValueInt(), AlignmentStyles, 1)])
        AddEmptyOption()
    ...
    elseif (a_page == "$mHUD_TargetsDataPage")
        ...
        _ingredientStyleOID_M = AddMenuOption("$mHUD_Ingredients", WidgetStyles2[GetValidArrayIndex(AHZIngredientsWidgetStyle.GetValueInt(), WidgetStyles2, DefaultIngredientsStyle)])
        _effectsStyleOID_M = AddMenuOption("$mHUD_OtherEffects", WidgetStyles2[GetValidArrayIndex(AHZEffectsWidgetStyle.GetValueInt(), WidgetStyles2, DefaultEffectsStyle)])
    ...
    elseif ((a_page == "$mHUD_TargetsInventory") || (a_page == "$mHUD_TargetsCountPage"))
        ...
        _inventoryAlignmentOID_M = AddMenuOption("$mHUD_Alignment", AlignmentStyles[GetValidArrayIndex(AHZInventoryWidgetRightAligned.GetValueInt(), AlignmentStyles, 0)])
    ...
    elseIf a_page == "$mHUD_PresetPage"
        ...
        if ValidatePapyrusUtil
            ...
            AddEmptyOption()
        endIf
        if saConfigs && saConfigs.length > 0
            selectedConfig = GetValidArrayIndex(selectedConfig, saConfigs, 0)
...
event OnOptionSliderOpen(int a_option)
    ...
    if (a_option == _sliderIconSize_OID_S)
        ...
        SetSliderDialogDefaultValue(DefaultIconSize)
        SetSliderDialogRange(16, 30)
        SetSliderDialogInterval(1)
    endif  
...
event OnOptionMenuOpen(int a_option)
    ...
    if (a_option == _ingredientStyleOID_M)
        SetMenuDialogStartIndex(GetValidArrayIndex(AHZIngredientsWidgetStyle.GetValueInt(), WidgetStyles2, DefaultIngredientsStyle))
        SetMenuDialogDefaultIndex(DefaultIngredientsStyle)
        SetMenuDialogOptions(WidgetStyles2)
    endIf

    if (a_option == _effectsStyleOID_M)
        SetMenuDialogStartIndex(GetValidArrayIndex(AHZEffectsWidgetStyle.GetValueInt(), WidgetStyles2, DefaultEffectsStyle))
        SetMenuDialogDefaultIndex(DefaultEffectsStyle)
        SetMenuDialogOptions(WidgetStyles2)
    endIf   

    if (a_option == _bottomAlignmentOID_M)
        SetMenuDialogStartIndex(GetValidArrayIndex(AHZBottomWidgetRightAligned.GetValueInt(), AlignmentStyles, 1))
        SetMenuDialogDefaultIndex(1)
        SetMenuDialogOptions(AlignmentStyles)
    endif

    if (a_option == _inventoryAlignmentOID_M)
        SetMenuDialogStartIndex(GetValidArrayIndex(AHZInventoryWidgetRightAligned.GetValueInt(), AlignmentStyles, 0))
        SetMenuDialogDefaultIndex(0)
        SetMenuDialogOptions(AlignmentStyles)
    endif

    if (a_option == _activationModeOID_M)
        SetMenuDialogStartIndex(GetValidActivationModeIndex(AHZActivationMode.GetValueInt()))
        SetMenuDialogDefaultIndex(0)
...
event OnOptionMenuAccept(int a_option, int a_index)
    ...
    if (a_option == _ingredientStyleOID_M && IsValidArrayIndex(a_index, WidgetStyles2))
        AHZIngredientsWidgetStyle.SetValueInt(a_index)
        SetMenuOptionValue(a_option, WidgetStyles2[a_index])
    endIf

    if (a_option == _effectsStyleOID_M && IsValidArrayIndex(a_index, WidgetStyles2))
        AHZEffectsWidgetStyle.SetValueInt(a_index)
        SetMenuOptionValue(a_option, WidgetStyles2[a_index])
    endIf   

    if (a_option == _bottomAlignmentOID_M && IsValidArrayIndex(a_index, AlignmentStyles))
        AHZBottomWidgetRightAligned.SetValueInt(a_index)
        SetMenuOptionValue(a_option, AlignmentStyles[a_index])      
    endif   

    if (a_option == _inventoryAlignmentOID_M && IsValidArrayIndex(a_index, AlignmentStyles))
        AHZInventoryWidgetRightAligned.SetValueInt(a_index)
        SetMenuOptionValue(a_option, AlignmentStyles[a_index])      
    endif   

    if (a_option == _activationModeOID_M && IsValidActivationModeIndex(a_index))
...
function FISS_SAVE()
    ...
    fiss.saveInt("AHZShowBottomWidgetAlways", AHZShowBottomWidgetAlways.GetValueInt())
    ...
    fiss.saveInt("AHZShowInventoryCountWithZero", AHZShowInventoryCountWithZero.GetValueInt())
    ...
    fiss.saveFloat("AHZIconSize", AHZIconSize.GetValue())
...
function FISS_LOAD()
    ...
    if fiss.loadBool("fissExists")
        ...
        AHZShowBottomWidgetAlways.SetValueInt(fiss.loadInt("AHZShowBottomWidgetAlways"))
        ...
        AHZShowInventoryCountWithZero.SetValueInt(fiss.loadInt("AHZShowInventoryCountWithZero"))
        ...
        AHZIconSize.SetValue(fiss.loadFloat("AHZIconSize")) 
    endIf
...
state SaveCurrentConfigBN
    event OnSelectST()
        string file = "..\\moreHUD\\" + sConfigNameCurrent
            ...
            JSONUtil.SetPathIntValue(file, ".!AHZShowBottomWidgetAlways", AHZShowBottomWidgetAlways.GetValueInt())
            ...
            JSONUtil.SetPathIntValue(file, ".!AHZShowInventoryCountWithZero", AHZShowInventoryCountWithZero.GetValueInt())
...
state ConfigToLoadMN
    ...
    event OnMenuAcceptST(int index)
        if IsValidArrayIndex(index, saConfigs)
...
state LoadSelectedConfigBN
    event OnSelectST()
        ...
        SetTextOptionValueST("$mHUD_PresetApply")
            ...
            AHZShowBottomWidgetAlways.SetValueInt(JSONUtil.GetPathIntValue(file, ".!AHZShowBottomWidgetAlways", AHZShowBottomWidgetAlways.GetValueInt()))
            ...
            AHZShowInventoryCountWithZero.SetValueInt(JSONUtil.GetPathIntValue(file, ".!AHZShowInventoryCountWithZero", AHZShowInventoryCountWithZero.GetValueInt()))
            ...
            AHZShowEnemySoulLevel.SetValueInt(JSONUtil.GetPathIntValue(file, ".!AHZShowEnemySoulLevel", AHZShowEnemySoulLevel.GetValueInt()))
...
bool Function IsValidArrayIndex(int index, string[] values)
    return values && index >= 0 && index < values.length
EndFunction

int Function GetValidArrayIndex(int index, string[] values, int defaultIndex)
    if IsValidArrayIndex(index, values)
        return index
    elseif IsValidArrayIndex(defaultIndex, values)
        return defaultIndex
    endif
    return 0
EndFunction

bool Function IsValidActivationModeIndex(int index)
    return ActivationModes && index >= 0 && index < 3 && index < ActivationModes.length
EndFunction

int Function GetValidActivationModeIndex(int index)
    if IsValidActivationModeIndex(index)
        return index
    endif
    return 0
EndFunction
